...
bool has(const std::map<T_Key, T_Val> &target,
         types::const_copyable_t<T_Key> key) {
...
bool has(const std::map<T_Key, T_Val> &target,
         types::const_copyable_t<T_Key> key, const T_Rest... rest) {
...
auto get(const std::map<T_Key, T_Val> &target,
         types::const_copyable_t<T_Key> key, const T_Rest... rest) {
  auto it = target.find(key);
  if (it == target.end()) throw "Key not found";
...
void set(std::map<T_Key, T_Val> *target, types::const_copyable_t<T_Key> key,
         types::const_copyable_t<T_Val> val) {
...
void del(std::map<T_Key, T_Val> *target, types::const_copyable_t<T_Key> key) {
  target->erase(key);
}
...
std::map<T_Key, T_Val> create(const T_Key key, const T_Val val) {
  return {{key, val}};
}
 ...
 *  provided argument will be the set value. */
auto create(const T_Key key, const T_Rest... rest) {
  return create(key, create(rest...));
}
