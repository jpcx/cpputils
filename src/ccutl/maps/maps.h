...
bool has(const std::map<T_Key, T_Val> &target, const T_Key &key) {
...
T_Val get(const std::map<T_Key, T_Val> &target, const T_Key &key) {
  auto it = target.find(key);
...
auto get(const std::map<T_Key, T_Val> &target, const T_Key &key,
         const T_Rest... rest) {
  auto it = target.find(key);
  if (it == target.end()) throw "Key not found";
...
void set(std::map<T_Key, T_Val> *target, const T_Key &key, const T_Val &val) {
  (*target)[key] = val;
}
...
void set(std::map<T_Key, T_Val> *target, const T_Key &key,
         const T_Rest... rest) {
  set(&(*target)[key], rest...);
}
...
void del(std::map<T_Key, T_Val> *target, const T_Key &key) {
  target->erase(key);
}
